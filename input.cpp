...
/*------------------------------------------------------------------------------
  　グローバル変数
------------------------------------------------------------------------------*/
LPDIRECTINPUT8			g_pInput = NULL;					// DirectInputインターフェースへのポインタ
...
HRESULT InitInput( HINSTANCE hInstance, HWND hWnd )
{
	if( g_pInput == NULL )
	{
		// DirectInputインターフェースの作成（マウス、パッド、キーボード共通のインターフェース）
...
HRESULT InitKeyboard(HINSTANCE hInstance, HWND hWnd)
{
	...
	// 協調モードを設定（フォアグラウンド＆非排他モード）※超重要
	if( FAILED(g_pDevKeyboard->SetCooperativeLevel( hWnd,
												  ( DISCL_FOREGROUND | DISCL_NONEXCLUSIVE ) ) ) )	// 自分が裏にある時にキー入力を受け取るか（受け取らない）
	{
...
void UninitKeyboard( void )
{
	...
	{// 入力デバイス(キーボード)の開放
		...
		g_pDevKeyboard->Unacquire(); // ※セーフリリースマクロは使わない
...
void UpdateKeyboard( void )
{
	...
	// デバイスからデータを取得
	if( SUCCEEDED( g_pDevKeyboard->GetDeviceState( sizeof( aKeyState ), aKeyState ) ) )		// キープレス情報を取得
	{
